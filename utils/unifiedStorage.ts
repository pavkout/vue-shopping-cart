  ...
  try {
    ...
    if (__isNotNull(cache)) {
      ...
      if (__isNotNull(cacheParsed)) {
        ...
        if (cacheParsed.unit && cacheParsed.unit === 's') {
          // in second unit
          milisMultiplier = 1000;
        } else if (cacheParsed.unit && cacheParsed.unit === 'm') {
          // in minute unit
          milisMultiplier = 1000 * 60;
        } else if (cacheParsed.unit && cacheParsed.unit === 'h') {
          // in hour unit
          milisMultiplier = 1000 * 60 * 60;
        } else if (cacheParsed.unit && cacheParsed.unit === 'd') {
          // in day unit
          milisMultiplier = 1000 * 60 * 60 * 24;
        }

        const expiryInMilis =
          parseInt(cacheParsed.expiry, 10) * milisMultiplier;
